...
app.get("/success",checkLocation,(req,res)=>{
  const order=JSON.parse(req.cookies.order);
  const trans_id=Math.floor((Math.random() * 1000000000) + 1);
  ...
  res.cookie("transaction_id",trans_id);
    ...
    const q1="insert into bill(transaction_id,total_price,date,table_no,rid,status) values('"+trans_id+"',"+sum+",current_timestamp(),"+tno+","+rid+",'processing')";
    
    con.query(q1,(err,result)=>{
      if(err) throw err;
  })
  for (let i = 0; i < Object.keys(order).length; i++) {
    const q2="insert into orders(name,transaction_id,quantity,price,rid) values('"+order[i][0]+"','"+trans_id+"',"+order[i][2]+","+order[i][1]*order[i][2]+","+rid+")";
